...
async function verifyMagicLink(magicLinkFragmentIdentifier, userName, context) {
    ...
    try {
        ...
        ({ Attributes: dbItem } = await ddbDocClient.send(new UpdateCommand({
            TableName: requireConfig("dynamodbSecretsTableName"),
            Key: {
                userNameHash,
            },
            ...
            ConditionExpression: "attribute_exists(#userNameHash) AND attribute_exists(#signatureHash) AND #signatureHash = :signatureHash AND attribute_not_exists(#uat)",
            ExpressionAttributeNames: {
...
function assertIsMagicLinkRecord(msg) {
    if (!msg ||
        typeof msg !== "object" ||
        !("userNameHash" in msg) ||
